...
public class SqsMessageSender {
    ...
    public <T> void sendMessage(T messagePayload) {
        log.info("메시지 전송을 시도합니다. Queue: {}, Payload: {}", queueName, messagePayload);
        try {
            sqsTemplate.send(queueName, messagePayload);
            log.info("메시지 전송에 성공했습니다. Queue: {}", queueName);
        } catch (QueueDoesNotExistException e) {
            log.error("큐가 존재하지 않습니다 (재시도되지 않음). Queue: {}", queueName, e);
            throw new RuntimeException("SQS queue not found.", e);
        } catch (SqsException e) {
            log.error("전송 중 서버 측 오류가 발생했습니다 (재시도되지 않음). Status: {}, AWS Error: {}",
            ...
            log.error("메시지 전송 중 알 수 없는 오류가 발생했습니다. Queue: {}", queueName, e);
    ...
    public <T> void recover(SdkClientException e, T messagePayload) {
        log.error("네트워크 등의 문제로 메시지 전송 재시도에 모두 실패했습니다. Payload: {}", messagePayload, e);
